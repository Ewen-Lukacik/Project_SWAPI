...
const peoplePage = () => {
    const [people, setPeople] = useState<Vehicle[]>([]);
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 10;
    const [totalPages, setTotalPages] = useState(0);
    const image = imagePeople.images[0].vehicles;

    useEffect(() => {
        const fetchPeople = async () => {
            try {
                const res = await fetch(`https://swapi.dev/api/vehicles/?page=${currentPage}`);
            ...
            <section>
                <div className="container flex flex-wrap justify-center">
                    {people.map((vehicle, index) => (
                        <a href={`lukacike/vehicles/${vehicle.url.split('/')[5]}`} key={index}>
                            <div className="cardbox bg-slate-200 m-10 w-64 min-h-48 rounded-lg flex flex-col">
                                ...
                                <div className="flex flex-col items-center pt-6 pb-6">
                                    <h1 className="text-gray-200 text-xl mb-4">{vehicle.name}</h1>
                                </div>
                            </div>
                        </a>
                    ))}
